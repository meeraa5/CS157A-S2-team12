	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try (Connection con = MySQLCon.getConnection()) {
			...
			if ("signup".equalsIgnoreCase(mode)) {
				...
				/*
				 * New signup users are regular users by default.
				 */
			...
			try (PreparedStatement stmt = con.prepareStatement(loginSql)) { //The blocks executes the login query using provided email and password to check making sure it conects with database
				...
				try (ResultSet rs = stmt.executeQuery()) {
					if (rs.next()) {
						...
						// Check the administrators table to see if this user_id is an admin
						String role = "user";
						String adminCheckSql = "SELECT user_id FROM administrators WHERE user_id = ?";
						
						try (PreparedStatement adminStmt = con.prepareStatement(adminCheckSql)) {
							adminStmt.setInt(1, userId);
							try (ResultSet rsAdmin = adminStmt.executeQuery()) {
								if (rsAdmin.next()) {
									role = "admin";
								}
							}
						...
						// DEBUG LINE: Check Eclipse Console for this output!
						System.out.println("LOG: User " + userId + " logged in with role: " + role);
						 ...
						 * Admin users are redirected to admin.jsp.
						 * Normal users are redirected to index.jsp.
						 */
